...
const hasChildrenEntries = (arr, character) => {
  const elements = Object.entries(arr);
  let response = false;
  for (let i = 0; i < elements.length; i++) {
    if(elements[i][1].name === character && elements[i][1].children.length > 0) {
      response = true;
    }
  }
  return response;
...
const totalCharacters = (arr) => {
  let sum = 0;
  Object.values(arr).forEach(element => {
    sum++; //for main character
    if(element.spouse) sum++;
...
const houseSize = (arr) => {
  ...
  Object.values(arr).forEach(element => {
    let size = 1;
    let houseObject = {};
    if(element.spouse) size++;
    size += element.children.length;
    houseObject.house = element.house;
    houseObject.members = size;
    sizes.push(houseObject);
  });
  return sizes;
};
