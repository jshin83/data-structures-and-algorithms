...
const findHappiness = (arr) => {
  // Solution code here...
  return arr.filter( element => element.includes(':)')).map( element => element);
};
...
const standardizePhoneNumbers = (arr) => {
  // Solution code here...
  return arr.map( element => element.substring(element.indexOf('(') + 1, element.indexOf(')')) + (element.substring(element.indexOf(' ') + 1, element.indexOf('-'))) + ( element.substring( element.indexOf('-') + 1, element.length)));
};
...
const onlyOddChars = (str) => {
  // Solution code here...
  //return str.split('').map( (letter, index) => index%2 === 1? letter : '').join(''); // solution when passing in index
  return str.split('').map( letter => str.indexOf(letter) % 2 === 1 ? letter : '').join('');
};
...
const allHappy = (arr) => {
  ...
  let result = true;
  arr.forEach( element => {
    if(!element.includes(':)')) result = false;
  });
  return result;
};
...
const findAnything = (arr, target) => {
  ...
  let results = [];
  arr.filter( element => {
    if(element.includes(target)) results.push(element);
  });
  return results;
};
...
const findEvery = (arr, target) => {
  ...
  let result = true;
  arr.forEach( element => {
    if(!element.includes(target)) result = false;
  });
  return result;
};
...
const unenrollBrook = (arr) => {
  // Solution code here...
  return arr.map( array => array.filter( element => !element.includes('Brook')).map(element => element));
};
...
const sortByDay = (arr) => {
  // Solution code here...
  const result = new Array(daysOfWeek.length).fill([]);
  daysOfWeek.forEach( (day, dayIndex) => result[dayIndex] = arr.filter( element => element.includes(day)));
  return result;
};
...
const characterByIndex = (arr) => {
  // Solution code here...
  return arr.map( (element, index) => element.substring(index, index + 1));
};
