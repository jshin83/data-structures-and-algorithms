...
const isCapitalized = (str) => {
  // Solution code here...
  //const replacePunc = /[^\w\s]/g;
  //replacePunc.replace(str);
  /*const split = str.split(' ');
  const cap = /^[A-Z]/;
  split.forEach(element => {
    if(!cap.test(element)) split.splice(split.indexOf(element), 1);
  });
  return split;*/
  //return str.match(replacePunc);
  let arr = [];
  arr = str.match(/[A-Z]\w+/g);
  if(!arr){
    return [];
  }
  return arr;
...
const citiesAtoJ = (arr) => {
  ...
  const regex = /^[A-J]/;
  arr.forEach(element => {
    if(regex.test(element)) newArr.push(element);
  });
  return newArr;
};
...
const matchMonth = (input) => {
  // Solution code here...
  const octRegex = /^[Oo]ct(ober)?$/;
  return octRegex.test(input);
};
