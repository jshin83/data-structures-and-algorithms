...
const count = ( target, input ) => {
  ...
  let count = 0;
  input.forEach( array => {
    return array.filter( arrElement => arrElement === target ? count += 1 : count);
  });
  return count;
};
...
const totalSum = (input) => {
  ...
  let count = 0;
  input.forEach( array => {
    count += array.reduce( ( acc, currentValue ) => acc + currentValue );
    return count;
  });
  return count;
};
...
let findShortest = (data) => {
  // Solution code here... two options - filter or reduce
  // let shortest = {
  //   name: '',
  //   height: 999
  // };
